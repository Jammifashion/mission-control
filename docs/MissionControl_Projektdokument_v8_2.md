...
**Version:** 8.2
**Letzte Aktualisierung:** Juni 2026 (Sprint 5.6 Audit-Fixes + Festpreis-Erweiterungen)
...
| `POST /api/festpreis/verkaeufe/sync` | festpreis-portal.js | Festpreis-Sync (on-hold + Storno) |
| `DELETE /api/festpreis/abrechnungen/:id` | festpreis-portal.js | FP Abrechnung (Entwurf) löschen |
| `POST /api/partner/:id/intern` | partner-artikel.js | Interne Bestellung anlegen (Partner + FP) |
...
| `frontend/partner.html` | GitHub Pages | Partner-Portal Lizenzmodell (öffentlich) |
| `frontend/partner-festpreis.html` | GitHub Pages | Partner-Portal Festpreismodell (öffentlich) |
...
- ✓ FP Auszahlungsformel auf VK-Netto-Basis korrigiert (`9d0fbc5`, `9d2af44`)
- ✓ FP Handlingskosten je Artikelkategorie konfigurierbar (`903755a`)
- ✓ FP Abrechnungen speichern Artikel-Aufschlüsselung als JSON (`79dfc3d`)
- ✓ FP Entwürfe löschbar (DELETE-Endpunkt + UI-Button) (`26212e2`)
- ✓ FP Storno-Zeilen rot markiert in MC + Kundenportal (`29c0d8c`, `de06bd0`)
- ✓ FP Abrechnungslogik header-tolerant (kein fixer Index mehr) (`5aed836`)
- ✓ FP Sync-Modal mit optionalem Startdatum (`f717bc2`)
- ✓ FP Kundenportal zeigt nur offene Bestellungen (`008b13e`)
- ✓ FP VK-Netto-Spalte (item.total) in FP_Verkäufe (`701f276`)
- ✓ FP Festpreise je Partner+Kategorie + Mehrkosten im Sync (`af8caae`)
- ✓ FP Artikel Entwürfe importieren + Kategorie-Dropdown (`122ab56`)
- ✓ FP Interne Bestellungen Tab eingeführt (`9d0fbc5`)
- ✓ FP Artikel-Zusammenfassung (summary-Endpunkt + Vorschau) (`16dd61e`)
- ✓ Debug-Skript `debug-festpreis-order.js` – Porto + Auszahlungsweg je Order (`6bd30fc`)
- ✓ Interne Bestellungen im Partnerportal editierbar (`5cd688f`)
...
| HonkShop Partner-Link | Link-Generierung muss `&shop=honk` anhängen – sonst Default jfn und keine Verkäufe sichtbar. Betroffene Partner brauchen frisch kopierten Link. |
| partnerIdExists statt loadPartner | POST /partner/:id/intern muss ID in Partner + FP_Partner suchen – loadPartner() kennt nur den Lizenz-Tab. |
| Festpreis Auszahlung | Basis ist VK-Netto (item.total), nicht Brutto. Handlingskosten je Kategorie konfigurierbar. |
| FP Abrechnungslogik | Header-basiert lesen statt fixer Spaltenindizes – robust gegen Sheet-Umstrukturierung. |
| Storno Festpreis-Sync | Historisch ohne after-Filter laden (fängt ältere Refunds). Negative Gegenbuchung in allen Geld-Spalten. Doppel-Schutz via Dedup-Key. |
...
# Festpreis-Berechnungsweg (Porto + Auszahlung) je Order
NODE_TLS_REJECT_UNAUTHORIZED=0 node backend/scripts/debug-festpreis-order.js <ORDER-ID>
...
## Offene Technische Schulden (Audit Mai 2026)

### Noch offen

| ID | Beschreibung | Datei | Priorität | Sprint |
|---|---|---|---|---|
| W2 | Race Condition ID-Generierung (KA-IDs, FPA-IDs) | anfragen-chat.js, festpreis-portal.js | SaaS-Blocker | vor SaaS |
| W3 | Partner-Tokens als URL-Query → landen in Logs | partnerView.js, festpreis-public.js | Mittel | 6 |
| ~~W4~~ | ~~Festpreis-Sync ohne on-hold + keine Storno-Gegenbuchung~~ | ~~festpreis-portal.js~~ | ✓ | ✓ |
| ~~W5~~ | ~~Legacy partner-view zeigt Entwurfs-Abrechnungen~~ | ~~partnerView.js~~ | ✓ | ✓ |
| ~~N1~~ | ~~Prompt-Caching Anthropic Chat-Calls~~ | ~~anfragen-chat.js~~ | ✓ | ✓ |
| ~~N2~~ | ~~WC-Kategorien/Attribute ungecacht~~ | ~~woocommerce.js~~ | ✓ | ✓ |
| ~~N3~~ | ~~Verbose console.log WC-Payloads entfernen~~ | ~~woocommerce.js~~ | ✓ | ✓ |
| ~~N4~~ | ~~/orders Multi-Status-Pagination korrigieren~~ | ~~woocommerce.js~~ | ✓ | ✓ |
| N5 | Automatisierte Tests einrichten | — | SaaS-Pflicht | vor SaaS |
| ~~N6~~ | ~~Prompt-Injection Chat absichern~~ | ~~anfragen-chat.js~~ | ✓ | ✓ |
| ~~N7~~ | ~~Secret-Cache Invalidierung~~ | ~~secrets.js~~ | ✓ | ✓ |

### Erledigt (Audit-Session Mai 2026)
- ✓ W1 trust proxy gesetzt (index.js)
- ✓ K1 Zeilenindex-Bug codebase-weit behoben (_sheetRow)
- ✓ K2 Festpreis-Abrechnung stabiler Match (entfiel durch K1-Fix)
- ✓ W6 PayPal auf Brutto-Basis korrigiert
- ✓ N1 Prompt-Caching Anthropic
- ✓ N2 WC-Kategorien gecacht
- ✓ N3 Verbose Logs entfernt
- ✓ N4 Pagination korrigiert
- ✓ N6 Prompt-Injection Schutz
- ✓ N7 Secret-Cache Invalidierung
- ⏳ N5 Automatisierte Tests → eigener Sprint
- ✓ Chat-Schutz: Rate-Limit, Honeypot, Input-Cap, Cloudflare Turnstile

### Erledigt (Sprint 5.6 / Juni 2026)
- ✓ W4 Festpreis-Sync: on-hold ergänzt + Storno-Gegenbuchung (negierte Qty + Geldbeträge, Doppel-Schutz) – `e1bc19c`
- ✓ W5 partnerView /abrechnungen filtert jetzt Status "entwurf" heraus (nur freigegeben + bezahlt sichtbar) – `c0624f0`
- ✓ FIX HonkShop-Partner sah keine Verkäufe: Link-Generierung hängt `&shop=honk` an – `f00d298`
- ✓ FIX Festpreis-Partner "nicht gefunden" bei interner Bestellung: `partnerIdExists()` sucht in Partner + FP_Partner – `333c906`

---

## Sprint 5.6: Agent-Wissen Tab

### Ziel
Eigener Reiter in Mission Control – alles was der Chat-Agent wissen soll an einem Ort pflegbar. Kein Code-Anfassen nötig für Wissens-Updates.

### Unter-Tabs

**Produktinfos (aus Sheet + editierbar)**
- Staffelpreise aus Kalkulation_Verkaufspreise direkt in MC bearbeiten
- Speichert zurück ins Sheet
- Druckinfos, Mindestmengen, Lieferzeiten

**Tonalität & Allgemeine Infos**
- Freitext-Editor: du/Sie, locker/förmlich
- Was wir nicht anbieten
- Häufige Fragen + Antworten
- Sonderregeln (Druckfarben egal etc.)

**System-Prompt Generator**
- Button "Prompt generieren (KI)" → Claude liest alle Daten + Freitext → optimierter System-Prompt
- Vorschau + manuell anpassbar
- Button "Aktivieren" → speichert als aktiven Agent-Prompt

**Knowledge Base Export**
- Button "KB exportieren" → erzeugt jammi-knowledge.md
- Wird beim Chat-Agent als Kontext geladen

### Außerdem in Sprint 5.6
- ~~W4~~: Festpreis-Sync on-hold + Storno-Gegenbuchung ✓
- ~~W5~~: Legacy partner-view Entwurfs-Filter ✓
- ~~N1~~: Prompt-Caching Anthropic ✓
- ~~N3~~: Verbose Logs entfernen ✓
- ~~N6~~: Prompt-Injection Schutz ✓

### 5.6c: Chef-Intake-Window + Partner-Eigenauftrag

**Scope (geklärt Juni 2026):**
- MC zeigt nur Anfragen aus zwei Quellen: Chat-Widget (`anfrage.html`) + Partner-Eigenaufträge
- Externe Kanäle (E-Mail, WhatsApp, Telefon) laufen über persönliche Agenten-App – kein MC-Eingang
- **Neu beschlossen:** Partner sollen Eigenaufträge direkt über ihr Portal (`partner.html` / `partner-festpreis.html`) einreichen können – statt per WhatsApp. Formular mit bekannter Partner-ID.
- Chef-Intake-Window = Prüf- und Bestätigungsschicht für diese zwei Quellen in MC

| Phase | Inhalt | Status |
|---|---|---|
| 5.6a | `Agent_Wissen` Sheet-Tab + Backend REST + MC-Tab (4 Sektionen) | ✓ |
| 5.6b | `agentWissenHelper.js` – KB + Staffelpreise, 5-min Cache, Chat-Flow | ✓ |
| 5.6c | Chef-Intake-Window + Partner-Eigenauftrag über Portal | ⏳ in Konzeption |
| 5.6d | Bestätigte Anfragen → Auftragsmonitor | ⏳ |
